...
if st.session_state.page == "main":
    ...
    with right:
        ...
        for label, key in {**CLASS_DATASETS, **REG_DATASETS}.items():
...
if st.session_state.get("compare_request"):
    ...
    if not saved_algs:
        ...
        # show detailed diagnostics per model (if predictions saved)
        st.subheader("Detailed diagnostics (if test-set predictions saved)")
        chosen = st.selectbox("Select model to inspect", ["(none)"] + saved_algs)
        if chosen != "(none)":
            preds_df = load_predictions_df(dkey, chosen)
            if preds_df is None:
                st.warning("No saved test-set predictions for this model.")
            else:
                st.write("Sample predictions:")
                st.dataframe(preds_df.head(8))
                # classification: confusion matrix
                if task == "Classification":
                    if "_true" in preds_df.columns and "_pred" in preds_df.columns:
                        try:
                            cm = confusion_matrix(preds_df["_true"], preds_df["_pred"])
                            fig = px.imshow(cm, text_auto=True, labels=dict(x="Predicted", y="Actual"), title=f"Confusion matrix — {chosen}")
                            st.plotly_chart(fig)
                    ...
                    if "_true" in preds_df.columns and "_pred" in preds_df.columns:
                        preds = preds_df["_pred"]
                        trues = preds_df["_true"]
                        residuals = trues - preds
                        fig = px.scatter(x=preds, y=residuals, labels={"x":"Predicted", "y":"Residuals"}, title=f"Residuals vs Predicted — {chosen}")
                        st.plotly_chart(fig)
